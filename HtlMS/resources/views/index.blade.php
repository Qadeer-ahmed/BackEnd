...
<body>
                ...
                <form action=" " method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" id="name" name="name" class="form-control"
                            value="{{ old('name', isset($hotel) ? $hotel->name : '') }}" required>
                    </div>

                    <div class="mb-3">
                        <label for="address" class="form-label">Address:</label>
                        <input type="text" id="address" name="address" class="form-control"
                            value="{{ old('address', isset($hotel) ? $hotel->address : '') }}" required>
                    </div>

                    <div class="mb-3">
                        <label for="location" class="form-label">Location:</label>
                        <input type="text" id="location" name="location" class="form-control"
                            value="{{ old('location', isset($hotel) ? $hotel->location : '') }}" required>
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="number" id="contact_number" name="contact_number" class="form-control"
                            value="{{ old('contact_number', isset($hotel) ? $hotel->contact_number : '') }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="email" class="form-label">email:</label>
                        <input type="email" id="email" name="email" class="form-control"
                            value="{{ old('email', isset($hotel) ? $hotel->email : '') }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="description" class="form-label">description:</label>
                        <input type="text" id="description" name="description" class="form-control"
                            value="{{ old('description', isset($hotel) ? $hotel->description : '') }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="rooms" class="form-label">rooms:</label>
                        <input type="number" id="rooms" name="rooms" class="form-control"
                            value="{{ old('rooms', isset($hotel) ? $hotel->rooms : '') }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="facilities" class="form-label">facilities:</label>
                        <input type="text" id="facilities" name="facilities" class="form-control"
                            value="{{ old('facilities', isset($hotel) ? $hotel->facilities : '') }}" required>
    ...
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"
        integrity="sha384-d7I/Dp8Mz9l2R9Xxjtd2iEUsKSW21fBh7a5zD1+2CyHQp1zLg2LCEUvx4zEDXSbH" crossorigin="anonymous">
    </script>
